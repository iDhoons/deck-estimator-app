...
export function buildCutPlan(plan: Plan, product: Product, rules: Ruleset): CutPlan | null {
  ...
  while (y <= scanMaxY) {
    ...
    if (requiredLenMm > 0) {
      totalRequiredLenMm += requiredLenMm;
  ...
  // 통계 계산
  const totalStockPieces = groupSeq - 1;
  const totalStockLengthMm = totalStockPieces * stockLen;
  const wasteLengthMm = Math.max(0, totalStockLengthMm - totalRequiredLenMm);
  const lossRate = totalStockLengthMm > 0 ? wasteLengthMm / totalStockLengthMm : 0;
  ...
  return {
    ...
    stats: {
      totalStockPieces,
      totalStockLengthMm,
      totalUsedLengthMm: totalRequiredLenMm,
      wasteLengthMm,
      lossRate,
    },
  };
