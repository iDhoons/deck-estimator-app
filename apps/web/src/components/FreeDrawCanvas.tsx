...
function applyOrthoSnap(
  ...
  const horizontalDeviation = Math.min(Math.abs(normalizedAngle), Math.abs(Math.abs(normalizedAngle) - 180));
  ...
  const verticalDeviation = Math.min(Math.abs(normalizedAngle - 90), Math.abs(normalizedAngle + 90));
...
export function FreeDrawCanvas({
  ...
  const [activeTool, setActiveTool] = useState<"delete" | null>(null);

  // Guide lines state
  const [guideLines, setGuideLines] = useState<Array<{ x1: number; y1: number; x2: number; y2: number }>>([]);

  // History 저장 함수
  const saveToHistory = useCallback((newPoints: Point[], newIsClosed: boolean) => {
    setHistory((prev) => {
      const newHistory = prev.slice(0, historyIndex + 1);
      newHistory.push({ points: newPoints, isClosed: newIsClosed });
      return newHistory.slice(-50); // 최대 50개 유지
    });
  ...
  return (
    ...
    >
      <g transform={transformGroup}>
        ...
        {!isClosed && points.length > 0 && cursorPosition && (() => {
          const lastPoint = points[points.length - 1];
          ...
          if (points.length >= 2) {
            ...
            angle = calculateAngle(prevPoint, lastPoint, { xMm: cursorPosition.x, yMm: cursorPosition.y });
          }
          ...
          return (
            <>
              ...
              {angle !== null && (() => {
                const radius = 40;
                const prevPoint = points[points.length - 2];
                const angle1 = Math.atan2(prevPoint.yMm - lastPoint.yMm, prevPoint.xMm - lastPoint.xMm);
                const angle2 = Math.atan2(cursorPosition.y - lastPoint.yMm, cursorPosition.x - lastPoint.xMm);
        ...
        {points.map((p, i) => {
          ...
          if (points.length > 1) {
            ...
            if (len > 0) {
              labelOffset = { x: -(avgDir.x / len) * labelDistance, y: -(avgDir.y / len) * labelDistance };
            }
          }
