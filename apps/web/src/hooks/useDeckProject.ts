...
export function useDeckProject() {
    ...
    const updatePolygon = useCallback((updatedPolygon: Polygon) => {
        setProject((prev) => {
    ...
    const actions = {
        ...
        addCutout: () => {
            setProject((prev) => {
                ...
                const inside = (xMm: number, yMm: number) => isPointInsidePolygon({ x: xMm, y: yMm }, outer);
            ...
            setProject((prev) => {
                ...
                if (safeMeta.shape === "rectangle") {
                    ...
                    for (const p of prevHole) {
                        minX = Math.min(minX, p.xMm); maxX = Math.max(maxX, p.xMm);
                        minY = Math.min(minY, p.yMm); maxY = Math.max(maxY, p.yMm);
                    }
